...
{
    public static class ManufacturerTestService
    {
        public static Guid GetExistingId()
        {
            List<Guid> existingIds = ManufacturerManager.GetAll().Select(m => m.Id).ToList();
        ...
        {
            List<string> existingCodes = ManufacturerManager.GetAll().Select(m => m.Code).ToList();
            Random random = new Random();
